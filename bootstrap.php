...
try {
    ...
    $pdo->exec("INSERT IGNORE INTO user_permissions(user_id,permission) SELECT user_id,'barang_manage' FROM user_permissions WHERE permission='unit_manage'");
    $pdo->exec("DELETE FROM user_permissions WHERE permission='unit_manage'");
    ...
    if (!$hasVisitIdentity) {
        $pdo->exec("ALTER TABLE room_visits ADD identitas VARCHAR(120) NULL AFTER nama");
    }
    $hasVisitPurpose = $pdo->query("SHOW COLUMNS FROM room_visits LIKE 'keperluan'")->fetch();
    if (!$hasVisitPurpose) {
        $pdo->exec("ALTER TABLE room_visits ADD keperluan VARCHAR(255) NULL AFTER ruangan");
    }
    $pdo->exec("INSERT IGNORE INTO user_permissions(user_id,permission) SELECT u.id,p.permission FROM users u JOIN (SELECT 'dashboard_view' permission UNION ALL SELECT 'barang_view' UNION ALL SELECT 'units_view' UNION ALL SELECT 'scan_view') p ON u.role='user' LEFT JOIN user_permissions up ON up.user_id=u.id AND up.permission=p.permission WHERE up.user_id IS NULL");
    ...
    if (!$hasItemType) {
        $pdo->exec("ALTER TABLE barang ADD tipe_barang VARCHAR(20) NOT NULL DEFAULT 'umum' AFTER status");
    }
    $hasExpiry = $pdo->query("SHOW COLUMNS FROM barang LIKE 'tanggal_expired'")->fetch();
    if (!$hasExpiry) {
        $pdo->exec("ALTER TABLE barang ADD tanggal_expired DATE NULL AFTER tanggal_masuk");
    }
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS unit_status_logs (id INT AUTO_INCREMENT PRIMARY KEY,unit_id INT NOT NULL,barang_id INT NOT NULL,old_status VARCHAR(20) NOT NULL,new_status VARCHAR(20) NOT NULL,user_id INT NOT NULL,keterangan VARCHAR(255) NULL,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,KEY unit_status_logs_created (created_at),CONSTRAINT fk_unit_status_log_unit FOREIGN KEY(unit_id) REFERENCES barang_unit(id) ON DELETE CASCADE,CONSTRAINT fk_unit_status_log_barang FOREIGN KEY(barang_id) REFERENCES barang(id) ON DELETE CASCADE,CONSTRAINT fk_unit_status_log_user FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE) ENGINE=InnoDB");
    foreach (["nama_unit VARCHAR(150) NULL AFTER kode_unit", "lokasi_id INT NULL AFTER nama_unit", "spesifikasi TEXT NULL AFTER lokasi_id", "kondisi ENUM('Baik','Rusak Ringan','Rusak Berat') NOT NULL DEFAULT 'Baik' AFTER spesifikasi", "catatan TEXT NULL AFTER kondisi", "tanggal_expired DATE NULL AFTER catatan"] as $column) {
        ...
        if (!$pdo->query("SHOW COLUMNS FROM barang_unit LIKE '$columnName'")->fetch()) {
            $pdo->exec("ALTER TABLE barang_unit ADD $column");
        }
    }
...
function admin_only()
{
    ...
    $permission = null;
    if (basename($_SERVER['SCRIPT_NAME'] ?? '') === 'units.php') {
        $permission = 'unit_manage';
    }
    if (basename($_SERVER['SCRIPT_NAME'] ?? '') === 'index.php' && ($_POST['action'] ?? '') === 'save_transaksi') {
        $permission = 'transaction_manage';
    }
    if ($permission && has_permission($permission)) {
        return;
    }
    flash('danger', 'Akses hanya untuk fitur yang diizinkan Admin.');
...
function has_permission($permission)
{
    ...
    $impliedPermissions = ['barang_view' => ['barang_manage'],'units_view' => ['unit_manage','barang_manage'],'unit_manage' => ['barang_manage'],'history_view' => ['transaction_manage'],'dashboard_view' => ['barang_manage','transaction_manage','category_manage','location_manage']];
    foreach ($impliedPermissions[$permission] ?? [] as $implied) {
        if (has_direct_permission($implied)) {
            return true;
        }
    }
...
function permission_only($permission)
{
    ...
    if (!has_permission($permission)) {
        flash('danger', 'Anda belum mendapat izin untuk fitur ini.');
        header('Location: '.url('index.php'));
        exit;
    }
}
...
function expiry_status($date)
{
    ...
    if ($days < 0) {
        return ['label' => 'Kedaluwarsa', 'class' => 'danger', 'days' => $days];
    }
    if ($days <= 30) {
        return ['label' => 'Segera kedaluwarsa', 'class' => 'warning', 'days' => $days];
    }
...
{
    global $pdo;
    if ($oldStatus === $newStatus || !logged_in()) {
        return;
    }
...
function sync_barang_units($barangId, $kode, $targetStock, $barangStatus = 'Tersedia')
{
    ...
    $unitStatus = unit_status_from_barang_status($barangStatus);
    if ($targetStock > $current) {
        $insert = $pdo->prepare('INSERT INTO barang_unit(barang_id,kode_unit,nama_unit,lokasi_id,tanggal_expired,status) VALUES(?,?,?,?,?,?)');
        for ($number = $current + 1; $number <= $targetStock; $number++) {
            $unitCode = $kode.'-'.str_pad((string)$number, 3, '0', STR_PAD_LEFT);
            $insert->execute([$barangId, $unitCode, $masterData['nama'].' '.$number, $masterData['lokasi_id'], $masterData['tanggal_expired'], $unitStatus]);
        }
        ...
        if (count($ids) < $current - $targetStock) {
            throw new Exception('Stok tidak dapat dikurangi karena sebagian unit sudah berstatus keluar atau rusak.');
        }
        $delete = $pdo->prepare('DELETE FROM barang_unit WHERE id=?');
        foreach ($ids as $unitId) {
            $delete->execute([$unitId]);
        }
    }
    $fillEmpty = $pdo->prepare("UPDATE barang_unit SET nama_unit=COALESCE(NULLIF(nama_unit,''), CONCAT(?, ' ', SUBSTRING_INDEX(kode_unit, '-', -1))), lokasi_id=COALESCE(lokasi_id, ?), tanggal_expired=COALESCE(tanggal_expired, ?), status=CASE WHEN status='tersedia' THEN ? ELSE status END WHERE barang_id=?");
    $fillEmpty->execute([$masterData['nama'], $masterData['lokasi_id'], $masterData['tanggal_expired'], $unitStatus, $barangId]);
}
