...
// Menampilkan daftar akun atau menyimpan perubahan izin akun yang dipilih.
admin_only();
...
echo '<script>document.addEventListener("DOMContentLoaded",function(){const form=document.querySelector("form");if(!form)return;const boxes=[...form.querySelectorAll("input[name=\"permissions[]\"]:not([type=\"hidden\"])")];if(!boxes.length)return;const wrapper=document.createElement("div");wrapper.className="form-check border rounded p-3 mb-3";const all=document.createElement("input");all.type="checkbox";all.className="form-check-input";all.id="select-all-permissions";const label=document.createElement("label");label.className="form-check-label fw-semibold";label.htmlFor=all.id;label.textContent="Pilih semua izin";wrapper.append(all,label);form.querySelector(".row")?.before(wrapper);const sync=function(){all.checked=boxes.every(function(box){return box.checked});all.indeterminate=!all.checked&&boxes.some(function(box){return box.checked})};all.addEventListener("change",function(){boxes.forEach(function(box){box.checked=all.checked});sync()});boxes.forEach(function(box){box.addEventListener("change",sync)});sync()});</script>';
