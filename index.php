...
if (is_post()) {
    ...
    try {
        ...
        if ($action === 'save_transaksi') {
            admin_only();
            $id = (int)$_POST['barang_id'];
            $jumlah = (int)$_POST['jumlah'];
            $jenis = $_POST['jenis'];
            if (!$id || $jumlah < 1 || !in_array($jenis, ['masuk','keluar'])) {
            ...
            }$pdo->prepare('INSERT INTO transaksi(barang_id,jenis,jumlah,tanggal,keterangan,user_id) VALUES(?,?,?,?,?,?)')->execute([$id,$jenis,$jumlah,$_POST['tanggal'],trim($_POST['keterangan']),$_SESSION['user']['id']]);
            $pdo->commit();
