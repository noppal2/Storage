...
if(logged_in()) permission_any('barang_view', 'units_view');
...
if(is_post()){
	...
	if(!in_array($status,['tersedia','keluar','terpakai','rusak'],true))$status='tersedia';
	$expired=trim($_POST['tanggal_expired']??'')?:null;
	if($expired!==null && !preg_match('/^\d{4}-\d{2}-\d{2}$/',$expired))$expired=null;
...
if($barang){
	...
	$params=[$barangId];
	if($search!==''){$sql.=' AND (b.nama LIKE ? OR b.kode LIKE ? OR bu.kode_unit LIKE ? OR bu.nama_unit LIKE ? OR bu.catatan LIKE ?)';$term='%'.$search.'%';$params=array_merge($params,[$term,$term,$term,$term,$term]);}
	$sql.=' ORDER BY bu.id';
	$s=$pdo->prepare(str_replace('FROM barang_unit bu','FROM barang_unit bu JOIN barang b ON b.id=bu.barang_id',$sql));$s->execute($params);$units=$s->fetchAll(PDO::FETCH_ASSOC);
}
...
<div class="row g-3">
<?php foreach($units as $unit): $qr=qr_image_url($unit['kode_unit'],240); $unitExpiryDate=$unit['tanggal_expired']?:($barang['tanggal_expired']??null); $unitExpiry=expiry_status($unitExpiryDate); $isExpired=$unitExpiry&&$unitExpiry['label']==='Kedaluwarsa'; $unitDisplayStatus=$unit['status']; ?>
