...
if (!$profile) { session_destroy(); header('Location: '.url('login.php')); exit; }
...
if (is_post()) {
    ...
    try {
        if ($action === 'update_profile') {
            ...
            $bio = trim($_POST['bio'] ?? '');
            if (strlen($nama) < 3) throw new Exception('Nama minimal 3 karakter.');
            if (!preg_match('/^[a-zA-Z0-9_.-]{3,50}$/', $username)) throw new Exception('Username hanya boleh huruf, angka, titik, garis bawah, atau minus.');
            if ($email !== '' && !filter_var($email, FILTER_VALIDATE_EMAIL)) throw new Exception('Format email belum valid.');
            if ($telepon !== '' && !preg_match('/^[0-9+() .-]{6,30}$/', $telepon)) throw new Exception('Format nomor telepon belum valid.');
            if (strlen($bio) > 255) throw new Exception('Bio maksimal 255 karakter.');
        ...
        if ($action === 'update_password') {
            ...
            if (!password_verify($current, (string)$stored->fetchColumn())) throw new Exception('Password lama tidak sesuai.');
            if (!password_is_strong($new)) throw new Exception('Password baru minimal 8 karakter dan harus berisi huruf besar, huruf kecil, serta angka.');
            if ($new !== $confirm) throw new Exception('Konfirmasi password tidak sama.');
        ...
        if ($action === 'update_password') $passwordError = $e->getMessage(); else $profileError = $e->getMessage();
    }
    $stmt->execute([$userId]);
